...
def download_invoice_pdf(request, invoice_id):
    ...
    title_style = ParagraphStyle('InvoiceTitle', parent=styles['Heading1'], fontName='Helvetica-Bold', fontSize=24, leading=28, textColor=PRIMARY, spaceAfter=5)
    brand_style = ParagraphStyle('BrandText', fontName='Helvetica-Bold', fontSize=12, leading=16, textColor=TEXT_DARK)
    section_hdr_style = ParagraphStyle('SectionHeader', fontName='Helvetica-Bold', fontSize=9, leading=13, textColor=TEXT_MUTED, spaceAfter=4)
    meta_val_style = ParagraphStyle('MetaValue', fontName='Helvetica', fontSize=9, leading=13, textColor=TEXT_DARK)
    meta_val_bold = ParagraphStyle('MetaValueBold', fontName='Helvetica-Bold', fontSize=9, leading=13, textColor=TEXT_DARK)
    body_normal = ParagraphStyle('BodyNormal', fontName='Helvetica', fontSize=9, leading=13, textColor=TEXT_DARK)
    body_bold = ParagraphStyle('BodyBold', fontName='Helvetica-Bold', fontSize=9, leading=13, textColor=TEXT_DARK)
    header_style = ParagraphStyle('TableHeader', fontName='Helvetica-Bold', fontSize=9, leading=13, textColor=colors.white)

    meta_content = f"<b>Invoice #:</b> {r['invoice_id']}<br/><b>Date:</b> {r['invoice_date']}<br/><b>Status:</b> {r['payment_status'].upper()}<br/><b>Method:</b> {r['payment_method']}"
    if r.get('due_date'):
    ...
    line_table.setStyle(TableStyle([('BACKGROUND', (0,0), (-1,-1), PRIMARY), ('BOTTOMPADDING', (0,0), (-1,-1), 0), ('TOPPADDING', (0,0), (-1,-1), 0)]))
    story.append(line_table)
    story.append(Spacer(1, 15))

    address_data = [
        [Paragraph("FROM STORE", section_hdr_style), Paragraph("TO CUSTOMER", section_hdr_style)],
        [Paragraph(f"<b>{r['store_name']}</b><br/>{r['store_address'] or ''}", meta_val_style), Paragraph(f"<b>{r['customer_name']}</b><br/>{r['customer_address'] or ''}", meta_val_style)]
    ]
    address_table = Table(address_data, colWidths=[260, 260])
    address_table.setStyle(TableStyle([('VALIGN', (0,0), (-1,-1), 'TOP'), ('BOTTOMPADDING', (0,0), (-1,-1), 10)]))
    story.append(address_table)
    story.append(Spacer(1, 10))

    detail_headers = [Paragraph("VEHICLE MODEL", section_hdr_style), Paragraph("VIN / CHASSIS NUMBER", section_hdr_style), Paragraph("SALES AGENT", section_hdr_style)]
    detail_vals = [Paragraph(r['vehicle_name'], meta_val_bold), Paragraph(r['vin'] or '—', meta_val_style), Paragraph(f"{r['employee_name']}<br/><font color='#64748B'>{r['employee_role'] or 'Staff'}</font>", meta_val_style)]
    detail_table = Table([detail_headers, detail_vals], colWidths=[180, 180, 160])
    detail_table.setStyle(TableStyle([('VALIGN', (0,0), (-1,-1), 'TOP'), ('BACKGROUND', (0,0), (-1,-1), BG_LIGHT), ('GRID', (0,0), (-1,-1), 0.5, BORDER_COLOR), ('TOPPADDING', (0,0), (-1,-1), 8), ('BOTTOMPADDING', (0,0), (-1,-1), 8), ('LEFTPADDING', (0,0), (-1,-1), 10), ('RIGHTPADDING', (0,0), (-1,-1), 10)]))
    ...
    fin_headers = [Paragraph("DESCRIPTION", header_style), Paragraph("REFERENCE DETAILS", header_style), Paragraph("AMOUNT", header_style)]
    fin_rows = [
        fin_headers,
    ...
    fin_table.setStyle(TableStyle([
        ('BACKGROUND', (0,0), (-1,0), colors.HexColor('#0F172A')), ('VALIGN', (0,0), (-1,-1), 'MIDDLE'), ('TOPPADDING', (0,0), (-1,0), 10), ('BOTTOMPADDING', (0,0), (-1,0), 10),
        ('LINEBELOW', (0,1), (-1,1), 0.5, BORDER_COLOR), ('LINEBELOW', (0,2), (-1,2), 0.5, BORDER_COLOR),
        ('TOPPADDING', (0,1), (-1,-1), 10), ('BOTTOMPADDING', (0,1), (-1,-1), 10), ('LEFTPADDING', (0,0), (-1,-1), 10), ('RIGHTPADDING', (0,0), (-1,-1), 10),
        ('ALIGN', (2,0), (2,-1), 'RIGHT'), ('BACKGROUND', (0,3), (-1,3), colors.HexColor('#EEF2F6')), ('LINEBELOW', (0,3), (-1,3), 1.5, PRIMARY), ('TOPPADDING', (0,3), (-1,3), 12), ('BOTTOMPADDING', (0,3), (-1,3), 12)
    ]))
    ...
    footer_divider.setStyle(TableStyle([('BACKGROUND', (0,0), (-1,-1), BORDER_COLOR), ('BOTTOMPADDING', (0,0), (-1,-1), 0), ('TOPPADDING', (0,0), (-1,-1), 0)]))
    story.append(footer_divider)
    story.append(Spacer(1, 10))

    thank_you_style = ParagraphStyle('ThankYouText', fontName='Helvetica-Oblique', fontSize=10, leading=14, textColor=TEXT_MUTED, alignment=1)
